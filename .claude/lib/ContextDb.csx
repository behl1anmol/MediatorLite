...
//   * Schema is idempotent: runs .claude/db/schema.sql on every call (CREATE IF NOT EXISTS).
//   * Session ID resolution order:
//       1. MEDIATORLITE_SESSION_ID env var (set within same process, e.g. Cursor / direct script).
//       2. .claude/db/.current-session file (cross-process persistence for Claude Code hooks,
//          where each hook invocation is a separate process and env vars do not survive).
//       3. Create a new session (writes both the env var and the file).
...
public static class ContextDb
{
    ...
    private static void EnsureInitialised()
    {
        ...
        lock (_lock)
        {
            ...
            var dbDir = Path.Combine(claudeDir, "db");
    ...
    {
        // Walk upward from CWD until we find a .claude/ directory or .git/.
        var dir = new DirectoryInfo(Directory.GetCurrentDirectory());
        while (dir is not null)
        {
            if (Directory.Exists(Path.Combine(dir.FullName, ".claude"))) return dir.FullName;
    ...
    public static string EnsureSession(string? userRequest = null, string? branch = null)
    {
        ...
        // 1. Check env var (set within same process).
        ...
        // 2. Check the persistence file (cross-process access for Claude Code hooks,
        //    where each hook invocation is a separate process).
        var sessionFile = Path.Combine(Path.GetDirectoryName(_dbPath)!, ".current-session");
        if (File.Exists(sessionFile))
        {
            var fileSid = File.ReadAllText(sessionFile).Trim();
            if (!string.IsNullOrWhiteSpace(fileSid) && SessionExists(fileSid))
            {
                Environment.SetEnvironmentVariable("MEDIATORLITE_SESSION_ID", fileSid);
                return fileSid;
            }
        }
        ...
        // Persist to env var AND the session file for cross-process access.
        Environment.SetEnvironmentVariable("MEDIATORLITE_SESSION_ID", sid);
        File.WriteAllText(sessionFile, sid);
    ...
    public static void CloseSession(string id, string status = "closed")
    {
        ...
        // Clear the session file so the next hook invocation starts a fresh session.
        EnsureInitialised();
        var sessionFile = Path.Combine(Path.GetDirectoryName(_dbPath)!, ".current-session");
        if (File.Exists(sessionFile))
    ...
    {
        var sid = Environment.GetEnvironmentVariable("MEDIATORLITE_SESSION_ID");
        // Also try the session file when env var is not set (Claude Code cross-process scenario).
        if (string.IsNullOrWhiteSpace(sid))
        {
            try
            {
                ...
                var sessionFile = Path.Combine(Path.GetDirectoryName(_dbPath)!, ".current-session");
                if (File.Exists(sessionFile))
                    sid = File.ReadAllText(sessionFile).Trim();
            }
            catch { /* best-effort */ }
        }
        using var conn = OpenConnection();
