...
agents live under [`.claude/agents/`](./agents/). The orchestrator is the single entry point
...
flowchart TD
    ...
    SM  -->|reads & writes| DB[(SQLite DB<br/>.claude/db/session.sqlite)]
    BE  -->|reads & writes| DB
    FE  -->|reads & writes| DB
    T   -->|reads & writes| DB
    DO  -->|reads & writes| DB
...
[`.claude/db/schema.sql`](./db/schema.sql)). The convention is:
