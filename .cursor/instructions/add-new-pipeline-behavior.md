...
2. **Choose `[BehaviorOrder]`**. Lower values wrap outer; higher values wrap inner (closer to the handler). The existing conventions from the sample and tests:
   ...
   > Note: validation behaviors emitted by the generator always run first for validated request types, regardless of `[BehaviorOrder]` on other behaviors. See [.cursor/rules/20-source-generator.mdc](.cursor/rules/20-source-generator.mdc).
