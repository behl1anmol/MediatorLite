...
- You have read [.cursor/rules/20-source-generator.mdc](.cursor/rules/20-source-generator.mdc) and understand the `IIncrementalGenerator` contract: the pipeline must be composed of equatable record/struct nodes, `CreateSyntaxProvider` must be cheap, and `RegisterSourceOutput` is the only place that writes source.
