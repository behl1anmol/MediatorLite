...
3. **`backend-developer`: fix.** Keep the fix minimal and local:
   ...
   - Preserve the invariants in [.cursor/rules/10-dispatch-invariants.mdc](.cursor/rules/10-dispatch-invariants.mdc). Specifically, do not reintroduce reflection in [Mediator.cs](src/MediatorLite/Internal/Mediator.cs):
