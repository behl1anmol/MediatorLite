...
3. **`backend-developer`: fix.** Keep the fix minimal and local:
   ...
   - Preserve the invariants in [.cursor/rules/10-dispatch-invariants.mdc](.cursor/rules/10-dispatch-invariants.mdc). Specifically, do not reintroduce reflection or a runtime dispatch layer — the generated `SourceGeneratedMediator` implements `IMediator` directly via a fully-typed type-pattern switch (shape emitted by `HandlerDiscoveryGenerator`):
   ...
   - Do not alter the public API of `AddMediatorLite()` or introduce new overloads (see [.cursor/rules/00-project-conventions.mdc](.cursor/rules/00-project-conventions.mdc)).
