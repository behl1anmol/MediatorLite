...
#
# Usage (called from .cursor/hooks.json):
#   pwsh -NoProfile -File .cursor/hooks/run-hook.ps1 <hook-file.csx> [extra args]
#
# Responsibilities:
#   1. Locate dotnet-script (install it on first run if missing).
#   2. Resolve the .csx path relative to .cursor/hooks/.
