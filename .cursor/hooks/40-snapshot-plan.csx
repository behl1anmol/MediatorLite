...
// .cursor/hooks/40-snapshot-plan.csx
// Event: afterPlanCreation
// Purpose: mirror every plan created via the CreatePlan tool into the `plans` table
//          so the Scrum Master agent (and any other) can query recent plans.
//
// Payload: Cursor writes plans to .cursor/plans/<slug>_<hash>.plan.md. We do not
// receive a stdin payload reliably across builds, so this hook walks .cursor/plans/
...
try
{
    ...
    var plansDir = Path.Combine(repoRoot, ".cursor", "plans");
