...
namespace MediatorLite.Tests.Reflection
{
    ...
    {
        [Fact]
        public void RegisterHandlersFromAssembly_AddsAssembly()
        {
            var options = new MediatorOptions();
            var assembly = typeof(MediatorOptionsTests).Assembly;

            var result = options.RegisterHandlersFromAssembly(assembly);

            Assert.Same(options, result);
        }

        [Fact]
        public void RegisterHandlersFromAssembly_WithNull_ThrowsArgumentNullException()
        {
            var options = new MediatorOptions();

            var ex = Assert.Throws<ArgumentNullException>(() => options.RegisterHandlersFromAssembly(null!));
            Assert.NotNull(ex);
        }

        [Fact]
        public void RegisterHandlersFromAssembly_WithDuplicate_DoesNotAddTwice()
        {
            var options = new MediatorOptions();
            var assembly = typeof(MediatorOptionsTests).Assembly;
        ...
        [Fact]
        public void RegisterHandlersFromAssemblyContaining_AddsAssemblyOfType()
        {
            var options = new MediatorOptions();

            var result = options.RegisterHandlersFromAssemblyContaining<MediatorOptionsTests>();
        ...
        [Fact]
        public void AddMultipleAssemblies_ChainedCalls_Work()
        {
            var options = new MediatorOptions();
            var assembly = typeof(MediatorOptionsTests).Assembly;
            ...
            var result = options
                ...
                .RegisterHandlersFromAssemblyContaining<MediatorOptionsTests>();
